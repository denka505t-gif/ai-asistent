# MEMORY.md — долгосрочная память Агента

> Что Агент знает обо мне на длинной дистанции: профиль, проекты, инфраструктура, стек, предпочтения. Читается в начале каждой сессии. Лимит — 200 строк.

---

## Кто я

- **Имя:** Денис
- **Роль:** Директор дорожно-строительной компании
- **Чем занимаюсь:** Руковожу дорожно-строительной компанией, интересуюсь нейросетями, занимаюсь частными инвестициями
- **Хобби:** сам активно рыбачит (спиннинг) — отсюда личный интерес к проекту Fishing Assistant. В июле 2026 был на рыбалке в г. Севан (Армения, озеро Севан)

## Контакты и аккаунты

> Дописывайте по мере появления. Telegram, GitHub, email, ключевые аккаунты сервисов.

*Пока пусто.*

---

## Активные проекты

1. **plan-fact** — сопоставление плана и факта строительных работ, анализ отклонений. В разработке, архитектура готова. Источник истины — `~/projects/plan-fact/CLAUDE.md`
2. **Технологические карты** — составление карт на выполнение дорожно-строительных работ
3. **Агент для планирования путешествий** — личный помощник по планированию поездок
4. **Fishing Assistant** — Telegram-бот помощника рыболова, **в бою**: `@Fish_Assist_Rus_bot` (сценарии 1–3 работают, сценарий 4 «фото→вид» заморожен). Источник истины по статусу — `~/projects/fishing-assistant/CLAUDE.md` на сервере, история разработки — `~/projects/fishing-assistant/docs/HISTORY.md` там же. Локальная копия `c:\Users\dell\Documents\projects\fishing-assistant\` может отставать — синхронизации нет
5. **Транскрибатор аудиофайлов** — расшифровка → протоколы совещаний (идея)
6. **Поиск лотов на банкротных торгах** — авто и земля (идея)
7. **Динамический алкометр** — расчёт BAC во времени (идея)
8. **Анализ инвестиционных обзоров** — (идея)

---

## Инфраструктура

> Сюда дописывайте: серверы (VPS, IP, доступы), домены, базы данных, ключевые шаблоны deploy-команд. Чтобы Агент мог работать с инфраструктурой не переспрашивая.

- **Доступ к Claude сейчас:** через VPN (не через российский реселлер/прокси). Это значит: любой VPS для агента должен иметь IP за пределами России (ЕС), иначе Claude Code на сервере не подключится к Anthropic — VPN на сервере не поднимаем, это лишняя точка отказа.
- **VPS для агента (развёрнут 2026-07-08):** Beget, сервер "Tranquil Azamat", IP 31.129.109.129, Ubuntu 24.04, 1 ядро/2 ГБ RAM/15 ГБ NVMe. Root-доступ по SSH (пароль у Дениса, в файлы не сохраняем).
- **Что установлено на сервере:** Node.js 20, Claude Code CLI (авторизован под denka505t@gmail.com, подписка Pro), VS Code CLI + VS Code Tunnel (постоянный systemd-сервис, привязан к GitHub-аккаунту Дениса). Файлы Агента (все 9 DNA-файлов, memory/, knowledge/, .claude/settings.json+skills/) лежат в `/home/agent/workspace/`, симлинк `/home/agent/CLAUDE.md` → `workspace/CLAUDE.md`.
- **Telegram-бот на сервере:** установлен в `/home/agent/.agent/bot/`, токен в `/home/agent/.agent/.env`, запущен как systemd-сервис `agent-bot` (автозапуск включён). Бот ждёт первый `/start` для авто-привязки к владельцу.
- **Подключение к серверу с компьютера Дениса — только Remote-SSH:** VS Code → F1 → «Remote-SSH: Connect to Host» → `agent-server` → папка `/home/agent/workspace/`. Вход по ключу `~/.ssh/id_ed25519` (denis-laptop), прописан в `/root/.ssh/authorized_keys`. Tunnel (Tunnels → agent) на этом ноутбуке не использовать — вероятная причина в VPN v2RayTun (TUN-режим перехватывает локальный WebSocket-мост 127.0.0.1), отключать VPN нельзя (нужен для Claude Code). Переустановка VS Code не помогает. Разбор: `knowledge/vscode-tunnel-troubleshooting.md`.
- **Graphify установлен для `bot/`** (2026-07-20): `pipx`-пакет `graphifyy` под пользователем `agent`, граф кода в `bot/graphify-out/`, обновлять после правок кода — `graphify update .`. Диск сервера чистится автоматически по cron (`/root/scripts/vscode-cleanup.sh`, воскресенье 4:00) — держит места достаточно.
- **Auto-mode классификатор Claude Code блокирует на этом сервере: установку hook'ов (`graphify claude install`), правку `.claude/settings.json` для самому-себе-разрешений, добавление cron-задач** — даже после явного разрешения в диалоге. Для таких шагов сразу даю Денису готовую команду для ручного запуска по SSH, не пытаюсь обойти.

---

## Стек и инструменты

- **Сервер:** Node.js 20, Grammy (Telegram-боты), Claude Code CLI, systemd-сервисы
- **Ноутбук:** VS Code + Remote-SSH, Git Bash, PuTTY/plink (SSH из скриптов), winget
- **AI:** Claude по подписке Pro (OAuth, denka505t@gmail.com) — на ноутбуке и на сервере

---

## Предпочтения

- **Обращение:** Вы
- **Стиль ответов:** Подробно, с примерами и пошаговой логикой. Денис только начинает работать с агентами, поэтому важно объяснять детально
- **Язык общения:** Русский
- **Что бесит в общении ИИ:**
  - «Конечно!», «Отличный вопрос!», «Давайте разберёмся!»
  - Длинные ответы из 5+ пунктов, когда хватит трёх
  - Повторение вопроса пользователя перед ответом
  - «Если нужна помощь — обращайтесь»
  - Извинения вместо обновления понимания
  - Размытые «может быть, возможно», когда нужен «да/нет»

---

## Установленные скиллы и плагины

Плагины (глобально, работают): superpowers 6.1.1, frontend-design. Скиллы — актуальный список в `.claude/skills/README.md`. Копии superpowers-скиллов в `.claude/skills/` отключены локально через settings.local.json как дубли живого плагина (2026-07-15). **Файлы копий не удалять** — они нужны серверу, где плагин не работает и трудятся именно копии.

---

## Ключевые решения

- **Статус проекта живёт только в CLAUDE.md проекта.** В MEMORY.md/PROJECTS.md — одна строка со ссылкой. Синхронизировано с серверным Агентом (2026-07-15)
- **Код и DNA-файлы между ноутбуком и сервером не синхронизируются автоматически.** Перенос — только через git или вручную, в обе стороны
- **2026-07-21:** ручная сверка памяти с серверным Агентом (сервер молчал с 15.07 — Telegram-ботом никто не пользовался). Ноутбук получил: хобби Дениса (рыбалка, Севан), ссылку на HISTORY.md. Сервер получил: Graphify на bot/, ограничение auto-mode классификатора, актуальный статус plan-fact
- **Fishing Assistant, сценарий 4 (фото→вид)** заморожен решением Дениса — не поднимать без явного запроса
- **VPS — только с IP вне России**, VPN на сервере не поднимаем (см. «Инфраструктура»)

---

## Ссылки на knowledge/

- `knowledge/claude-reauth-manual.md` — бот отвечает пустыми сообщениями / Claude CLI молчит → переавторизация. Читать при сбоях бота
- `knowledge/update-guide.md` — как обновить Telegram-бота до новой версии
- `knowledge/vscode-tunnel-troubleshooting.md` — VS Code Tunnel не подключается (диск сервера забит / зависшая служба / подозрение на VPN на ноутбуке)
- `knowledge/specs/2026-07-07-fishing-assistant.md` — исходное ТЗ Fishing Assistant
- `knowledge/2026-07-15-audit-report-from-server.md` — отчёт серверного Агента об аудите 15.07 (выводы уже применены на ноутбуке в тот же день, файл скопирован для истории при синхронизации 21.07)

---

## Правила работы с этим файлом (для Агента)

- **Дописывай в конец нужного раздела**, не переписывай файл целиком.
- **Файл ≤ 200 строк.** Если разрастается — консолидируй: удали устаревшее, объедини дубликаты, длинные блоки выноси в отдельный файл `knowledge/`.
- **Дневные заметки** идут в `memory/YYYY-MM-DD.md`, не сюда. Здесь только то, что пригодится через месяц.
- **Большие справочники** идут в `knowledge/`, не сюда. Здесь только короткая ссылка.
- **Перед каждой сессией** — перечитай этот файл. Без него ты не Ваш Агент, а обычный чат.
- **Если узнал постоянный факт о человеке** (новый проект, новая инфраструктура, изменение предпочтения) — сразу зафиксируй в нужном разделе.
